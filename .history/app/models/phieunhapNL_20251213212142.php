...
class PhieuNhapNL {
    ...
    function layDanhSachNguyenLieu() {
        $sql = "SELECT n.manguyenlieu, k.tennguyenlieu, n.soluongton, n.makho, k.soluongnguyenlieu 
                FROM nguyenlieu n 
    ...
    // Lưu phiếu nhập
    // -------------------------------
    function luuPhieuNhap($maKho, $ngayNhap, $maNguoiLap, $trangThai, $maNguyenLieu, $soLuong, $soLuongTonKho) {
    $conn = $this->db->conn;
    if (!$conn) {
    ...
    $sql = "INSERT INTO phieunhapkhonl
            (makho, ngaynhap, maNguoiLap, trangthai, manguyenlieu, soluong, soluongtonkho)
            VALUES
            ('{$maKho}', '{$ngayNhap}', '{$maNguoiLap}', '{$trangThai}', '{$maNguyenLieu}', {$soLuong}, {$soLuongTonKho})";
